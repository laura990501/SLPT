...
if __name__ == '__main__':
    ...
    net = Face_Detector.YuFaceDetectNet(phase='test', size=None)
    ...
    for bbox in dets:
        bbox[0] = int(bbox[0] / 320.0 * frame.shape[1] + 0.5)
        bbox[2] = int(bbox[2] / 320.0 * frame.shape[1] + 0.5)
        bbox[1] = int(bbox[1] / 240.0 * frame.shape[0] + 0.5)
        bbox[3] = int(bbox[3] / 240.0 * frame.shape[0] + 0.5)
        ...
        confidence = bbox[4]  # 获取人脸框的置信度

        face_locations.append((int(leftmost), int(topmost), int(rightmost), int(bottommost)))
    ...
    result_path = "result_single_img/result2.jpg"
    cv2.imwrite(result_path, frame)

    for i in range(len(face_locations)):
        leftmost, topmost, rightmost, bottommost = face_locations[i]
        confidence = face_confidences[i]
        print("Face", i+1, "Location:", leftmost, topmost, rightmost, bottommost)
        print("Face", i+1, "Confidence:", confidence)